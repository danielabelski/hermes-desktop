...
The screen is organized as two tabs: Providers and Auxiliary Tasks. There is no longer a standalone Models tab — models are managed only under each provider card (see "Models live under each provider" below), since the provider owns the base URL and key a per-model editor would otherwise duplicate. The Auxiliary Tasks tab renders [[src/renderer/src/components/AuxiliaryTasksSection.tsx]] (per-task model overrides), and a **Browse Registry** button in the model-section header opens [[src/renderer/src/components/RegistryBrowserModal.tsx]] to pick curated models into the library.
